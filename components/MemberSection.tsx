      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {members.map((member, idx) => (
          <motion.div
            ...
            className="group p-8 rounded-[3rem] border border-champagne-100 bg-white hover:bg-champagne-50/50 transition-all duration-500 flex flex-col relative overflow-hidden"
          >
            {/* Background Decorative Element */}
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-sage-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-700"></div>

            <div className="flex items-center space-x-6 relative z-10">
              <div className="w-20 h-20 rounded-3xl bg-sage-50 flex items-center justify-center overflow-hidden shrink-0 shadow-inner border border-champagne-50">
                <img
              ...
              <div className="flex-grow">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center space-x-2">
                    <h4 className="text-2xl font-sans font-bold text-sage-600">{member.name}</h4>
                    {/* Badge Rendering */}
                  ...
                  <span className="text-[10px] text-sage-300 font-mono">2023.01</span>
                </div>

                <div className="flex items-center justify-between">
                  <p className="text-[10px] text-sage-400 uppercase tracking-widest font-bold">{member.role}</p>
                  {member.phone && (
                    <a
                      href={`tel:${member.phone}`}
                      className="p-2 rounded-xl bg-sage-50 text-sage-400 hover:bg-sage-600 hover:text-white transition-all duration-300 shadow-sm"
                    >
                      <Phone size={12} />
                    </a>
                  )}
            ...
            <div className="grid grid-cols-3 gap-2 mt-8 relative z-10">
              <div className="flex flex-col bg-white/60 backdrop-blur-sm px-3 py-3 rounded-2xl border border-champagne-50 group-hover:border-sage-200 transition-colors">
                <span className="text-[7px] uppercase tracking-widest text-sage-300 font-extrabold mb-1">Handicap</span>
                <span className="text-xs font-sans font-bold text-sage-600">HC {member.handicap}</span>
              </div>
              <div className="flex flex-col bg-white/60 backdrop-blur-sm px-3 py-3 rounded-2xl border border-champagne-50 group-hover:border-sage-200 transition-colors">
                <span className="text-[7px] uppercase tracking-widest text-sage-300 font-extrabold mb-1">Avg Score</span>
                <span className="text-xs font-sans font-bold text-sage-600">{member.averageScore}</span>
              </div>
              <div className="flex flex-col bg-white/60 backdrop-blur-sm px-3 py-3 rounded-2xl border border-champagne-50 group-hover:border-sage-200 transition-colors">
                <span className="text-[7px] uppercase tracking-widest text-sage-300 font-extrabold mb-1">Rounds</span>
                <span className="text-xs font-sans font-bold text-sage-600">{member.rounds}회</span>
              </div>
            </div>

            <div className="mt-6 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              <span className="text-[9px] uppercase tracking-widest text-sage-300 font-bold">Class of 2009</span>
              <div className="flex space-x-1">
